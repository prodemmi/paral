...
// DAGNode represents a node in the DAG (task or command)
type DAGNode struct {
	ID       string                 `json:"id"`
	Label    string                 `json:"label"`
	Type     string                 `json:"type"` // "task" or "command"
...
func (d *DAG) formatFunctions(command *runtime.Command) []map[string]interface{} {
	var result []map[string]interface{}
...
func (d *DAG) formatCommandForMetadata(cmd *runtime.Command) map[string]interface{} {
...
func (d *DAG) extractEnvironments(command *runtime.Command, matrix []string) map[string]string {
...
func (d *DAG) extractCondition(command *runtime.Command) string {
	// Extract from Functions
	for _, f := range command.Functions {
...
func (d *DAG) processCommandForDAG(command *runtime.Command, jobName string, matrixIdx, cmdIdx int, matrix []string, stash map[string]interface{}, dag *DAG, jobInstanceID string, seenNodes map[string]bool) (string, error) {
	// Determine command text to display
	var displayText string
	if command.RawText != "" {
		displayText = d.resolveCommand(command.RawText, matrix)
	...
	commandMetadata := map[string]interface{}{
		...
		"functions":   d.formatFunctions(command),
	}
	...
	if envs := d.extractEnvironments(command, matrix); len(envs) > 0 {
...
func (d *DAG) GenerateDAG() *DAG {
	...
	// Process each task
	for _, task := range d.Runtime.Jobs {
		...
		for matrixIdx, matrix := range matrixValues {
			...
			for i, command := range task.Commands {
				...
				if condition := d.extractCondition(command); condition != "" {
			...
			// Process commands for this task instance
			var prevCommandID string
			for cmdIdx, command := range task.Commands {
				// Resolve command text with matrix values
				resolvedCMD := d.resolveCommand(command.RawText, matrix)

				// Create command node
				commandNodeID := fmt.Sprintf("cmd_%s_%d_%d", task.Name, matrixIdx, cmdIdx)
				if seenNodes[commandNodeID] {
					d.Runtime.Reporter.Warn(fmt.Sprintf("Duplicate command ID %s detected", commandNodeID), &task.Metadata)
					continue
				}
				seenNodes[commandNodeID] = true
				...
				// Add edge from task to its first command
				if cmdIdx == 0 {
					...
					if len(directive.Params) > 0 {
						if targetJob, ok := directive.Params[0].(string); ok {
							if targetJobIDs, exists := jobNodeIDs[targetJob]; exists {
								...
								d.Runtime.Reporter.Warn(fmt.Sprintf("dependall target %s not found", targetJob), &task.Metadata)
							}
						}
					}
				}
